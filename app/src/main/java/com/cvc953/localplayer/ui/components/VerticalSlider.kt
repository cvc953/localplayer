...
fun VerticalSlider(
    ...
    val layoutHeight = remember { mutableStateOf(0f) }
...
private fun drawVerticalSlider(
    ...
    with(scope) {
        ...
        // inactive track (thin)
        drawRoundRect(color = inactiveColor.copy(alpha = 0.25f), topLeft = Offset(trackLeft, trackTop), size = Size(trackPx, trackHeight), cornerRadius = CornerRadius(trackPx / 2f, trackPx / 2f))

        // active track (from thumb to bottom), draw thin overlay
        val activeTop = thumbY
        val activeHeight = trackBottom - activeTop
        drawRoundRect(color = activeColor.copy(alpha = 0.95f), topLeft = Offset(trackLeft, activeTop), size = Size(trackPx, activeHeight), cornerRadius = CornerRadius(trackPx / 2f, trackPx / 2f))
